...
//! 不是代码问题。判断方法：一台机器 `chuan send 文件`，另一台 `chuan discover`，
//! 后者应当列出前者，并显示和前者屏幕一致的验证码。

use std::time::Duration;

use chuanmen_core::qr::AddressHint;
use chuanmen_core::{verification_code, Advertisement, CancelToken};
...
async fn advertises_and_discovers_over_mdns() {
    ...
    let lan_ip = chuanmen_core::net::quic::local_address_hints(51234)
        .into_iter()
        .map(|h| h.host)
        .find(|h| h != "127.0.0.1")
    ...
    let hosts = chuanmen_core::discover(Duration::from_secs(5), None, &CancelToken::new())
        .await
        .expect("搜索附近设备失败");
