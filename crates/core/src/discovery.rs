...
pub const SERVICE_TYPE: &str = "_strangeroom._udp.local.";
...
pub fn verification_code(fingerprint: &str) -> String {
    let h = blake3::hash(format!("strange-room-verify:{fingerprint}").as_bytes());
    let b = h.as_bytes();
...
impl Advertisement {
    ...
    pub fn start(
        ...
        let instance = format!("sr-{sid}");
        let host_name = format!("sr-{}.local.", &sid[..sid.len().min(8)]);
